...
def prepare_sequence(seq, to_ix, gpu=False):
  if isinstance(to_ix, dict):
    ...
    # map(lambda w: to_ix[w] if w in to_ix, seq)
  elif isinstance(to_ix, list):
    idxs = [to_ix.index(w) if w in to_ix else 0 for w in seq]
    # idxs = map(lambda w: to_ix.index(w) if w in to_ix, seq)
...
def logSumExp(a, b):
  cdef np.ndarray[DTYPE_t, ndim=2] maxi = np.maximum(a, b, dtype=np.float64)
  cdef np.ndarray[DTYPE_t, ndim=2] aexp = a - maxi
  cdef np.ndarray[DTYPE_t, ndim=2] bexp = b - maxi
  cdef np.ndarray[DTYPE_t, ndim=2] sumOfExp = np.exp(aexp) + np.exp(bexp)
...
def logDot(a, b, redAxis=None):
  ...
  if redAxis==1:
    b = b.transpose()
  #   max_a = np.amax(a)
  #   max_b = np.amax(b, axis=0, keepdims=True)
  # else:
  #   max_a = np.amax(a, axis=1, keepdims=True)
  #   max_b = np.amax(b)

  cdef DTYPE_t max_a = np.amax(a)
  cdef DTYPE_t max_b = np.amax(b)

  cdef np.ndarray[DTYPE_t, ndim=2] C = np.dot(np.exp(a - max_a), np.exp(b - max_b))
...
def logMax(a, b, redAxis=None):
  ...
  cdef np.ndarray[DTYPE_t, ndim=1] max_a = np.amax(a)
  cdef np.ndarray[DTYPE_t, ndim=1] max_b = np.amax(b)

  cdef np.ndarray[DTYPE_t, ndim=3] C = np.max(np.exp(a[:, :, None]-max_a) * np.exp(b[None, :, :]-max_b), axis=1)
...
def logNormalize(a):
  ...
  cdef np.ndarray[DTYPE_t, ndim=1] denom = np.logaddexp.reduce(a, 1)
  return (a.transpose()- denom).transpose()

  # denom = -float('inf')
  # flattenedVec = a.flatten()
  # nElems = flattenedVec.shape[0]
  # normalizedVec = np.zeros(nElems)

  # for i in range(nElems):
  #   denom = logSumExp(denom, flattenedVec[i])

  # for i in range(nElems):
  #   normalizedVec[i] = flattenedVec[i] - denom

  # return normalizedVec.reshape(a.shape)
...
def logNormalizeTensor(a):
  ...
  # flattenedVec = a.view(a.numel())
  # nElems = flattenedVec.size(0)
  # normalizedVec = Variable(torch.zeros(nElems)).cuda()

  # denom = logSumExpTensor(flattenedVec)

  # for i in range(nElems):
  #   normalizedVec[i] = flattenedVec[i] - denom

  # return normalizedVec.view(a.size())
...
def computeF1(hyps, golds, prefix, labels_to_ix=None, baseline=False, write_results=False):
  ...
  if baseline:
    #hyps = [tag for tag in hyps]
    #golds = [dict(tag[:-1]) for tag in golds]
    #hyps = [unfreeze_dict(tag) for tag in hyps if tag not in labels_to_ix]
    #golds = [unfreeze_dict(tag) for tag in golds if tag not in labels_to_ix]
    for i, tag in enumerate(golds):
      #if tag not in labels_to_ix:
      hypsUnseen.append(unfreeze_dict(hyps[i]))
      goldsUnseen.append(unfreeze_dict(tag))
  else:
    hypsUnseen = hyps
    goldsUnseen = golds
  
  # calculate precision
  for i, word_tags in enumerate(hypsUnseen, start=0):
    for k, v in word_tags.items():
      ...
      if k in goldsUnseen[i]:
        if v==goldsUnseen[i][k]:
  ...
  # calculate recall
  for i, word_tags in enumerate(goldsUnseen, start=0):
    for k, v in word_tags.items():
      if v=="NULL":
        continue
  ...
  for k in f1_recall_scores.keys():
    ...
    # f1_average += f1_scores[k]

  # f1_average /= len(f1_scores)
